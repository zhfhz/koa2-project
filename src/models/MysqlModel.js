...
class MysqlModel extends Model {
  ...
  static async insert(...data){
    ...
    for(let i = 0; i<data.length; i++){
      //TODO: this assumes that Promise.all in MysqlDatabase that executes all the inserts will actually put them in the same order as they are passed. Is this always the case?
  ...
  static async query(sqlquery){
    if(!(sqlquery instanceof MysqlDatabase.SQLQuery)) throw new Error('The raw query must be an instance of MysqlDatabase.SQLQuery');
