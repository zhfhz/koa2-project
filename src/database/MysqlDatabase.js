...
class MysqlDatabase extends Database {
  constructor(config, inTransition) {
    super(config);
    if (inTransition) {
      console.log('创建s事务专用链接...')
      this.connection = mysql.createConnection(config);
    }
    Logger.info('Created a MysqlDatabase instance for a transaction');
  }
  ...
  async connect() {
    ...
    // getConnection mysql 不反回connection对象
    return await (async () => {
      return new Promise((resolve,reject) => {
        CONNECTIONS[configStr].getConnection(function(err, conn){
          if(err){
            throw err;
          }
          // connection.query默认不返回promise对象
  ...
  async disconnect() {
    const configStr = JSON.stringify(this.config);
    if(!CONNECTIONS[configStr]) return;
    Logger.info(`Closing ${CONNECTIONS[configStr]}`);
    await CONNECTIONS[configStr].end();
    delete CONNECTIONS[configStr];
  }

  static async disconnect(){
    Logger.info('Closing all MysqlDatabase connections for all instances');
  ...
  transaction(transactionFn){
    ...
    const transactionInstance = new MysqlDatabase(this.config, true);
